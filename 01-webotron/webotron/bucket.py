...
class BucketManager:
    ...
    def all_objects(self, bucket):
        """Get an iterator for all objects in in bucket."""
        return self.s3.Bucket(bucket).objects.all()

    def init_bucket(self, bucket_name):
        """Create ne bucket, or return an existing one"""

        s3_bucket = None

        try:
            s3_bucket = self.s3.create_bucket(Bucket=bucket_name, CreateBucketConfiguration={
